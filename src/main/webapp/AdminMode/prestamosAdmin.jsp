			...
			<nav class="bg-gray-50 border-b border-gray-200 p-4">
				<ul class="flex space-x-10 justify-center">
					...
					<li><a
						...
						class="hover:bg-blue-600 hover:text-white text-gray-700 font-semibold py-2 px-4 rounded-md focus:outline-none focus:shadow-outline transition duration-200 ease-in-out">Clientes</a></li>
					<li><a
						href="<%=request.getContextPath()%>/ServletListarCuentas?openListar=1&pagina=1"
						class="hover:bg-blue-600 hover:text-white text-gray-700 font-semibold py-2 px-4 rounded-md focus:outline-none focus:shadow-outline transition duration-200 ease-in-out">Cuentas</a></li>
					<li><a
						href="${pageContext.request.contextPath}/ServletPrestamosAdmi"
						class="bg-blue-600 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:shadow-outline transition duration-200 ease-in-out">Préstamos</a></li>
					<li><a href="/BancoParcial//AdminMode/reportesAdmin.jsp"
						class="hover:bg-blue-600 hover:text-white text-gray-700 font-semibold py-2 px-4 rounded-md focus:outline-none focus:shadow-outline transition duration-200 ease-in-out">Reportes</a></li>
				</ul>
			</nav>
			<%
...
%>
			<div class="p-6 flex-1 overflow-y-auto">
				...
				<form method="get"
					action="${pageContext.request.contextPath}/ServletPrestamosAdmi"
					class="flex items-center space-x-4">
					<label for="prestamos_autorizar"
						class="block text-gray-700 text-lg font-semibold whitespace-nowrap">Préstamos
						a Autorizar:</label>
					<% String filtroDni = (String) request.getAttribute("filtroDni"); %>
					<select id="prestamos_autorizar" name="dni"
						class="select-dni p-2 border border-gray-300 rounded-md bg-white">
    ...
    %>
						<option value="<%= dni %>"
							<%= dni.equals(filtroDni) ? "selected" : "" %>><%= dni %></option>
						<%
            }
        }
    }
    %>
					...
					<button type="submit"
						class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">Filtrar</button>
				</form>

				<% if (filtroDni != null && !filtroDni.isEmpty()) { %>
				<a href="${pageContext.request.contextPath}/ServletPrestamosAdmi"
					class="ml-4 text-red-600 font-semibold hover:underline">Quitar
					filtro</a>
				<% } %>
					...
					<table class="min-w-full divide-y divide-gray-200">
						<thead class="bg-red-700 text-white">
							<tr>
								...
								<th scope="col"
									class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Importe
									a Pagar</th>
								<th scope="col"
									class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Importe
									Pedido</th>
								<th scope="col"
									class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Plazo
									de Pago</th>
								<th scope="col"
									class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Monto
									por Mes</th>
								<th scope="col"
									class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Acciones</th>
							</tr>
						</thead>
...
%>
							<tr>
								<td
									class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= p.getIdPrestamo() %></td>
								...
								<td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
									<form method="post"
										...
										<input type="hidden" name="idPrestamo"
											...
											name="accion" value="aceptar" /> <input type="hidden"
											name="dni" value="<%= filtroDni != null ? filtroDni : "" %>" />
										<button
											class="icon-button bg-green-500 hover:bg-green-600 text-white"
											type="submit">&#10003;</button>
									</form>
									<form method="post"
										action="${pageContext.request.contextPath}/ServletPrestamosAdmi"
										...
										<input type="hidden" name="idPrestamo"
											value="<%= p.getIdPrestamo() %>" /> <input type="hidden"
											name="accion" value="rechazar" /> <input type="hidden"
											name="dni" value="<%= filtroDni != null ? filtroDni : "" %>" />
										<button
											class="icon-button bg-red-500 hover:bg-red-600 text-white"
											type="submit">&#10006;</button>
									</form>
								</td>
							</tr>
...
%>
				...
				<div
					...
					<% if (paginaActual > 1) { %>
					<a
						href="<%= baseUrl %>?pagina=<%= paginaActual - 1 %><%= queryParams %>"
