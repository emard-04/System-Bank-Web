...
public class ServletModificarCuentas extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("nroCuenta") != null) {
			...
			String json = "{" + "\"nroCuenta\": \"" + cuenta.getNroCuenta() + "\"," + "\"dniCliente\": \""
					+ cuenta.getUsuario().getPersona().getDni() + "\"," + "\"fechaCreacion\": \""
					+ cuenta.getFechaCreacion() + "\"," + "\"tipoCuenta\": \""
					+ cuenta.getTipoCuenta().getIdTipoCuenta() + "\"," + "\"tipoCuentaDescripcion\": \""
					+ cuenta.getTipoCuenta().getDescripcion() + "\"," + "\"cbu\": \"" + cuenta.getCbu() + "\","
		...
		try {
			...
			if (usuario == null) {
				response.sendRedirect(
						request.getContextPath() + "/AdminMode/cuentaAdmin_modificar.jsp?error=Usuario no encontrado");
				return;
			}
			Cuenta cuenta = new Cuenta();
			...
			e.printStackTrace();
			response.sendRedirect(
					request.getContextPath() + "/AdminMode/cuentaAdmin_modificar.jsp?error=Error inesperado");
		}
	}
